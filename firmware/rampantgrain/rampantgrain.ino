...
// and https://www.instructables.com/id/The-Arduino-Synthesizer/
//
...
// PWM_VALUE LED
#define FRQLED_PIN (13) // not as consistent as pin 13      

// mapmode LED
#define mapModeLED_PIN (6)    // the number of the LED pin  
...
uint16_t mapMidi(uint16_t input) {
  return (midiTable[map(input,0,1023,1,80)]);
}
...
uint16_t mapPentatonic(uint16_t input) {
  uint8_t value = (map(input,0,1023,1,37));
