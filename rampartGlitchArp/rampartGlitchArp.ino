...
void updateControl()
{
  ...
  // modulating within the audio loop

  alright = alright + 1;
  con = (thirdknob / 20) + 2; // hmmm, this and cons are tied and change over 16 instead of the initial 8

  // if alright is greater, slower, lesser, faster since the number of iterations on audioupdate
  if (alrightcons > 15) {
    alrightcons = 4;
  }
  ...
  if (swotch > 29 && swotch < 40)
  {
    ...
    centre_freq = centre_freq - con;
    //digitalWrite(LED_PIN, HIGH);
    //analogWrite(LED_PIN_1, fundamental);
    digitalWrite(LED_PIN_1, LOW);
  }
